    ...
    <div className="space-y-24 pb-20 bg-[#FAF6F0] text-[#2C2725]">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {INITIAL_SERVICES.slice(0, 4).map((service, idx) => (
            <div
              key={service.id}
              className="group bg-[#F3ECE3] rounded-3xl p-6 pt-10 border border-[#EAE3DA] hover:border-[#C1785A] shadow-card hover:shadow-warm-lg transition-all duration-300 flex flex-col justify-between relative overflow-hidden"
            >
              {service.is_popular && (
                <div className="absolute top-4 right-4 px-3 py-1 rounded-full bg-[#C1785A] text-[#FAF6F0] text-[10px] font-bold uppercase tracking-widest shadow-sm">
                  Popular
                </div>
              )}

              {/* Card Arch Top Icon */}
              <div className="w-16 h-16 rounded-2xl bg-[#FAF6F0] border border-[#EAE3DA] flex items-center justify-center mx-auto mb-6 text-[#C1785A] shadow-sm group-hover:scale-110 transition-transform">
                {idx === 0 && <Scissors className="w-7 h-7" />}
                {idx === 1 && <Sparkles className="w-7 h-7" />}
                {idx === 2 && <Award className="w-7 h-7" />}
                {idx === 3 && <Flame className="w-7 h-7" />}
              </div>

              {/* Service Info */}
              <div className="text-center space-y-2 mb-6">
                <span className="text-[11px] uppercase tracking-[0.2em] font-bold text-[#8C462C]">
                  {service.category}
                </span>
                <h3 className="font-serif text-xl font-bold text-[#2C2725] line-clamp-2">
                  {service.name}
                </h3>
                <p className="text-xs sm:text-sm text-[#6E6663] line-clamp-3 leading-relaxed">
                  {service.description}
                </p>
              </div>

              {/* Pricing & Booking CTA */}
              <div className="pt-4 border-t border-[#EAE3DA] space-y-3.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-serif font-extrabold text-lg text-[#2C2725]">
                ...
                <Link
                  ...
                  className="w-full py-3 rounded-full bg-[#C1785A] hover:bg-[#A86347] text-[#FAF6F0] text-xs sm:text-sm font-bold uppercase tracking-[0.15em] transition-all flex items-center justify-center gap-2 shadow-sm"
                >
